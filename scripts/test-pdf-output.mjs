/**
 * Test script: generate a PDF using the ACTUAL proposalPdfExport module.
 * We compile the TS file on the fly with tsx, then call generateProposalPdf.
 * Run: node scripts/test-pdf-output.mjs
 */
import { execSync } from "child_process";
import fs from "fs";
import path from "path";

// Build a small wrapper that imports the real module and runs it
const wrapper = `
...
fs.writeFileSync("scripts/_test-runner.ts", wrapper);

try {
  execSync("cd /home/ubuntu/proposalcraft && npx tsx scripts/_test-runner.ts", {
    stdio: "inherit",
    timeout: 180000,
  });
} catch (e) {
