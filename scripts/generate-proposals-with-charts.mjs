...
// Draw a pie chart for price breakdown
function drawPieChart(page, x, y, radius, laborCost, materialsCost) {
  const total = laborCost + materialsCost;
  const laborPercent = laborCost / total;
  const materialPercent = materialsCost / total;
  
  // Labor slice (blue)
  const laborAngle = laborPercent * Math.PI * 2;
  const laborEndX = x + radius * Math.cos(laborAngle - Math.PI / 2);
  const laborEndY = y + radius * Math.sin(laborAngle - Math.PI / 2);
  ...
  page.drawText(`Labor: $${laborCost.toLocaleString()} (${Math.round(laborPercent * 100)}%)`, {
    x: legendX + 18,
    y: legendY + 2,
    size: 9,
    color: rgb(0, 0, 0),
  ...
  page.drawText(`Materials: $${materialsCost.toLocaleString()} (${Math.round(materialPercent * 100)}%)`, {
    x: legendX + 18,
    y: legendY - 16,
    size: 9,
    color: rgb(0, 0, 0),
...
async function generateProposalWithChart(tradeType, sampleData) {
  ...
  // Draw pie chart
  drawPieChart(page, margin + 80, y - 60, 50, sampleData.laborCost, sampleData.materialsCost);
...
async function main() {
  ...
  for (const [tradeType, sampleData] of Object.entries(ENHANCED_SAMPLES)) {
    console.log(`Generating ${tradeType} sample proposal with chart...`);
    const pdfDoc = await generateProposalWithChart(tradeType, sampleData);
  ...
  console.log("\n✓ All sample proposals with charts generated successfully!");
